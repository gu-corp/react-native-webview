...
public class ErrorPageHandler: InternalSchemeResponse {
    ...
    // You can define other handler and import at here
...
@available(iOS 14.0, *)
@objc(ErrorPageHelper)
public class ErrorPageHelper: NSObject {
    
    @objc
    public func loadPage(_ error: NSError, forUrl url: URL, inWebView webView: WKWebView) {
        ...
        // NOTE: ignore this block code, if you want to handle CertificateError, uncomment it
        ...
        if let urlWithQuery = components.url {
            ...
            let page = InternalURL.authorize(url: urlWithQuery) {
                // A session restore page is already on the history stack, 
                // so don't load another page on the history stack.
                webView.replaceLocation(with: page)
            } else {
                // A new page needs to be added to the history stack
                // (i.e. the simple case of trying to navigate to an url for the first time and it fails,
                // without pushing a page on the history stack, the webview will just show the current page).
    ...
    @objc
    public func canHandleError(error: NSError) -> Bool {
